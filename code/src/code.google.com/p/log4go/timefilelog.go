...
func (w *TimeFileLogWriter) computeRollover(currTime time.Time) int64 {
	if w.firstRollover == true {
		...
		if w.when == "MIDNIGHT" {
			...
			result = (currTime.Unix()/w.interval + 1) * w.interval
...
func (w *TimeFileLogWriter) adjustRolloverAt() {
	...
	for newRolloverAt <= currTime.Unix() {
		...
		newRolloverAt = (newRolloverAt/w.interval + 1) * w.interval
...
// This is the FileLogWriter's output method implement io.Writer
func (w *TimeFileLogWriter) Write(p []byte) (n int, err error) {
	...
	//fmt.Printf("num:%d msg:%s", num, msg)
